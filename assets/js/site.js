...
(function () {
  ...
  /* ── Google reviews ──────────────────────────────────────────────────────
     Progressive enhancement: the panel is already correct in the HTML, and
     this only replaces it if the Worker returns live data. Any failure — no
     API key, no network, a Google outage — leaves the written version alone. */

  function stars(n) {
    var full = Math.round(n || 0);
    return new Array(6).join('★').slice(0, Math.max(0, full)) +
           new Array(6).join('☆').slice(0, Math.max(0, 5 - full));
  }

  function reviewCard(r) {
    var el = document.createElement('figure');
    el.className = 'tc-greview';

    var head = document.createElement('div');
    head.className = 'tc-greview__head';

    if (r.photo) {
      var img = document.createElement('img');
      img.className = 'tc-greview__avatar';
      img.src = r.photo;
      img.alt = '';
      img.loading = 'lazy';
    ...
    name.className = 'tc-greview__name';
    name.textContent = r.author;              // textContent, never innerHTML
    var meta = document.createElement('div');
    meta.className = 'tc-greview__meta';
    meta.textContent = stars(r.rating) + (r.when ? ' · ' + r.when : '');
    who.appendChild(name);
    who.appendChild(meta);
    head.appendChild(who);

    var body = document.createElement('blockquote');
    body.className = 'tc-greview__text';
  ...
  function loadReviews() {
    ...
    if (!panel || !endpoint || endpoint.indexOf('REPLACE-ME') !== -1) return;

    fetch(endpoint.replace(/\/$/, '') + '/reviews')
      ...
      .then(function (d) {
        ...
        if (typeof d.rating === 'number') {
          ...
          setText('grating', rating);
          setText('gstars', stars(d.rating));
          setText('gsr', rating + ' out of 5');
        }
        if (typeof d.count === 'number' && d.count > 0) {
          setText('gcount', d.count + (d.count === 1 ? ' review' : ' reviews') + ' on the Truecraft Google Business Profile.');
        }
        if (d.mapsUri) {
          var link = q('glink');
          if (link) link.href = d.mapsUri;
        }
        ...
        if (d.reviews && d.reviews.length) {
          ...
          list.className = 'tc-greviews';
          d.reviews.slice(0, 4).forEach(function (r) { list.appendChild(reviewCard(r)); });
          feed.appendChild(list);
        }
      })
      .catch(function () { /* keep the written panel */ });
  }
